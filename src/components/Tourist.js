...
function Tourist() {
  ...
  const [tourists, setTourists] = useState([]); //관광지 리스트 상태관리
  ...
  const fetchData = async () => {
    try {
      const datas = await getTour({ regionNumb, currentPage });
      const lists = datas.items.item; // 리스트
      const totalLists = datas.totalCount; // 총 리스트 갯수
      setTourists((prevData) => (isMobile ? [...prevData, ...lists] : lists)); //디바이스에 따라 다르게 업로드
