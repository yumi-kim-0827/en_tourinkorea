...
import { getFestival } from ".././data/api";
...
function Festival() {
  ...
  const [festivals, setFestivals] = useState([]); //festivals 리스트 상태관리
  const [totalList, setTotalList] = useState(0); //전체 리스트
  ...
  const fetchData = async () => {
    try {
      const datas = await getFestival({ regionNumb, currentPage });
      const lists = datas.items.item; // 리스트
      const totalLists = datas.totalCount; // 총 리스트 갯수
      setFestivals((prevData) => (isMobile ? [...prevData, ...lists] : lists)); //디바이스에 따라 다르게 업로드
              ...
              <li key={list.contentid}>
                <Card style={{ width: "18rem" }}>
                  ...
                  <Card.Body>
                    ...
                    <Card.Text>
                      <span>Upload : {list.modifiedtime}</span>
                      <span>Location : {list.addr1}</span>
                    </Card.Text>
                    <Button
